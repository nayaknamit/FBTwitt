...
function BirdHouse(params) {
	...
	function get_request_verifier(callback) {
		...
		if (Ti.Platform.osname == 'iphone' && cfg.show_login_toolbar) {
			...
			//var toolbar = Ti.UI.createToolbar({top:10});
			...
			//toolbar.items = [btnLogout,flexSpace,toolbarLabel,flexSpace,btnClose];
			//win.add(toolbar);

			win.leftNavButton = btnLogout;
			win.rightNavButton = btnClose;
			win.title = 'Login with Twitter';
			//win.navBarHidden=true;
			//webView.add(btnClose);
		...
		if (authorized === false) {
			...
			if (Ti.Platform.osname == 'iphone') {
				var winTW = Titanium.UI.createWindow({
					height : ((Ti.Platform.displayCaps.platformHeight * 0.5) - 20), // half because the keyboard takes up half
					width : (Ti.Platform.displayCaps.platformWidth - 0),
					top : 50,
					//right:10,
					//left:10,
					borderColor : '#224466',
				...
				var btnTW = Ti.UI.createButton({
					...
					width : 60,
					height : 30,
					top : 10, //((Ti.Platform.displayCaps.platformHeight*0.5)-75),
					right : 10
				});
				...
				var btnReset = Ti.UI.createButton({
					...
					width : 60,
					height : 30,
					top : ((Ti.Platform.displayCaps.platformHeight * 0.5) - 60),
					left : 10
				});
